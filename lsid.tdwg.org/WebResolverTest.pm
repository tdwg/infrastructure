...
sub session {
	...
	my $session = CGI::Session->new('driver:File',
					($query->cookie('LSIDWebResolverTest') || $query->param('LSIDWebResolverTest') || undef),
					{
						Directory=> '/var/tmp/'
					}
	...
	if (!$sess_id || !$query->cookie('LSIDWebResolverTest') || 
	    $query->cookie('LSIDWebResolverTest') ne $sess_id) {

		$self->header_props(-cookie=> $query->cookie(-name=> 'LSIDWebResolverTest',
							     -value=> $sess_id,
							     -path=> '/')
				);
	}
...
sub my_header {
	...
	my $rdf_link = $query->Link( { -rel  => 'alternate',
				       -href => "http://$ENV{HTTP_HOST}/$lsid",
				       -type => 'application/rdf+xml',
				       -title=> 'unprocessed RDF+XML metadata'
				       }
				     );

	my $header = $query->start_html(-title=> $TITLE,
...
sub start {
	...
	my $raw_examples = [];
	    ...
	    my $raw_link = "<a href='/$EXAMPLES[$i]'>http://$ENV{HTTP_HOST}/$EXAMPLES[$i]</a>";
	    
	    push @{ $examples }, { 'text'=> $link, 'button'=> '' };
	    push @{ $raw_examples }, { 'text'=> $raw_link, 'button'=> '' };

	    $i++;
        }
	
	$elements{'examples'} = $examples;
	$elements{'raw_examples'} = $raw_examples;
